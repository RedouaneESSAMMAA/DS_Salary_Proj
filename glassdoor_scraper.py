...
def get_jobs(num_jobs, verbose, path, slp_time):
    ...
    while len(jobs) < num_jobs:  #If true, should be still looking for new jobs.
        ...
        job_buttons = driver.find_elements(By.CLASS_NAME,"JobsList_jobsList__lqjTr")  #jl for Job Listing. These are the buttons we're going to click.
        for job_button in job_buttons: 
            ...
            if len(jobs) == 0:
                try:
                    driver.find_element(By.CSS_SELECTOR, '[class="CloseButton"]').click()
                except ElementClickInterceptedException:
                    pass
            ...
            while not collected_successfully:
                try:
                    company_name = driver.find_element(By.CSS_SELECTOR, '[class="EmployerProfile_employerName__qujuA"]').text
                    location = driver.find_element(By.CSS_SELECTOR, '[class="JobDetails_location__mSg5h"]').text
                    job_title = driver.find_element(By.CSS_SELECTOR, '[class="JobDetails_jobTitle__Xvsha"]').text
                    job_description = driver.find_element(By.CSS_SELECTOR, '[class="MobileActionBar_container__ZBdmS"]').text
                    collected_successfully = True
                except:
                    time.sleep(5)

            try:
                salary_estimate = driver.find_element(By.CSS_SELECTOR, '[class="SalaryEstimate_averageEstimate__xIgkL"]').text
            except NoSuchElementException:
                salary_estimate = -1 #You need to set a "not found value. It's important."
            
            try:
                rating = driver.find_element(By.CSS_SELECTOR, '[class="RatingHeadline_sectionRatingScoreLeft__di1of"]').text
            except NoSuchElementException:
